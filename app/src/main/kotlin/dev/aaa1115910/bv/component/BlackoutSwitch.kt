 ...
 * - 替代 AnimatedContent 的“双树转场”，在低性能 TV 盒子上减少掉帧
 ...
 * - switchDelayMillis 默认 0：主线不建议调大，它会**直接延长黑屏时间**，TV 上很容易被当成“卡死”。
 * - onSwitched 必须是 O(1) 的轻操作（只做“同步 displayed 状态”或发轻量事件）：禁止在里面做 IO/解析/同步等待/大计算/页面初始化，否则会把黑场拖长成“罚站”。
 * - 如果确实需要“逐次必达/每次切页都必须执行副作用”：不要用这个 eventually-consistent 的切换器；改用队列/Mutex 串行执行，或把副作用绑定到 requestedDrawerItem（并自行处理取消/幂等）。
 */
@Composable
fun <T> BlackoutSwitch(
    ...
    LaunchedEffect(targetState) {
        ...
        // 特判：fade=0 且无额外 delay 时，直接切状态，不走“黑一下”的路径
        ...
        // 低端盒子 fade=0ms 时直接 snap，避免 0ms 动画仍触发一次调度/挂起
