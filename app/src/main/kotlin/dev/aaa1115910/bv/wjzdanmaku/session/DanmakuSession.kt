package dev.aaa1115910.bv.wjzdanmaku.session
...
import dev.aaa1115910.bv.wjzdanmaku.DanmakuEngine
import dev.aaa1115910.bv.wjzdanmaku.DanmakuEngineInputEvent
import dev.aaa1115910.bv.wjzdanmaku.DanmakuInputStreamType
import dev.aaa1115910.bv.wjzdanmaku.SimpleDanmakuEngine
import dev.aaa1115910.bv.wjzdanmaku.DanmakuFilterBlockReason
import dev.aaa1115910.bv.wjzdanmaku.DanmakuFilterChain
import dev.aaa1115910.bv.wjzdanmaku.DanmakuFilterResult
import dev.aaa1115910.bv.wjzdanmaku.DanmakuItem
...
import dev.aaa1115910.bv.wjzdanmaku.TimelineWindow
import dev.aaa1115910.bv.wjzdanmaku.VodDanmakuSegmentIndex
...
class SimpleDanmakuSession(
    ...
    override fun attachLive(roomId: Long, input: DanmakuLiveInput, buffer: DanmakuLiveBuffer): LiveSessionHandle {
        return attachLive(roomId, { consumer -> input.collect(roomId, consumer) }, buffer)
    }
    ...
    private fun onLiveFlush(event: SessionEvent.Live.Flush): Int {
        ...
        val acceptedItems = mutableListOf<DanmakuItem>()
        var filteredCount = 0
        for (liveEvent in events) {
            val item = liveEvent.toDanmakuItem(event.roomId, event.sourceId, liveEvent.timestampMs) ?: continue
            when (val result = filterChain.evaluate(item, currentFilterRule)) {
                is DanmakuFilterResult.Accepted -> acceptedItems += item
                is DanmakuFilterResult.Rejected -> {
                    filteredCount += 1
